...
\section{Midterm Technical Report}
Prepare a clear, professional technical report in a standard single-column format. Use an 11- or 12-point readable font, 1-inch margins, numbered section headings, page numbers, figure and table captions, and consistent citations and references. The report may be created in Word, Google Docs, Overleaf, or another suitable editor, but it must be submitted as a PDF. The main report must be \textbf{at least 6 pages}, excluding references and an optional appendix. There is no strict maximum, but the report should remain focused and avoid unnecessary repetition. Include representative screenshots and other essential run evidence in the main report; place additional supporting evidence in the optional appendix. Include:
\begin{enumerate}
  \item \textbf{Title and team members}: project title, student names, selected platform, and model access path.
  \item \textbf{Executive Summary}: tests performed, main findings, and the most important limitation observed.
  \item \textbf{Introduction and Objectives}: purpose, scope, and team objectives.
  \item \textbf{Platform and Setup}: agent version, system/VM, model/provider, installation and verification steps.
  \item \textbf{Tool-Calling Architecture}: labeled high-level diagram, component roles, and numbered sequence for one real question.
  \item \textbf{Test Methodology}: exact prompts, enabled tools, expected observations, and evidence source.
  \item \textbf{Results and Evidence}: per-task table of actual behavior, tool names and outcomes, representative evidence, and discrepancies.
  \item \textbf{Discussion and Limitations}: visibility gaps, unresolved questions, and how evidence could be improved.
  \item \textbf{Conclusion}: what the team learned and what remains unresolved.
  \item \textbf{References}: consistent citations to platform and model documentation and other sources.
...
\begin{enumerate}
  \item The same team midterm technical report PDF.
  \item A share link to the editable report source (for example, Word, Google Docs, or Overleaf) with instructor access.
...
\begin{longtable}{p{0.29\linewidth}p{0.56\linewidth}r}
  ...
  Technical report and repository   & Required report structure, consistent citations, clear results, GitHub link/commit, reproducibility, safe evidence, and contribution/AI-use disclosure.      & 15           \\
